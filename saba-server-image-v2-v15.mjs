...
async function persistReferenceImage(dataUrl,meta={}){
  ...
  if(Buffer.byteLength(parts.base64,'base64')>25*1024*1024)throw Object.assign(new Error('Reference image is too large.'),{status:413});
  const ext=parts.mime==='image/jpeg'?'jpg':parts.mime==='image/webp'?'webp':parts.mime==='image/gif'?'gif':'png';
  const file=await toFile(Buffer.from(parts.base64,'base64'),String(meta.name||`saba-reference-${Date.now()}.${ext}`),{type:parts.mime});
  const uploaded=await client.files.create({file,purpose:'assistants'});
  return String(uploaded?.id||'')||null;
}
