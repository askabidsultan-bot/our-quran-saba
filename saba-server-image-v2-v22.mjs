...
app.get('/',(_req,res)=>res.json({ok:true,service:'SABA Universal AI',version:'V21-PRO-DOWNLOAD',model,keyConfigured:Boolean(client),authConfigured:Boolean(supabaseUrl&&supabaseAnonKey),guestLimitConfigured:true,visionEnabled:Boolean(client),attachmentEnabled:true,imageGenerationEnabled:Boolean(client),imageModel:IMAGE_MODEL,imageCloudPersistence:Boolean(admin)}));
app.get('/health',(_req,res)=>res.json({ok:true,service:'SABA Universal AI',version:'V21-PRO-DOWNLOAD',model,keyConfigured:Boolean(client),authConfigured:Boolean(supabaseUrl&&supabaseAnonKey),guestLimitConfigured:true,visionEnabled:Boolean(client),attachmentEnabled:true,imageGenerationEnabled:Boolean(client),imageModel:IMAGE_MODEL,timestamp:new Date().toISOString()}));
app.get('/api/saba/config',(_req,res)=>res.json({ok:true,version:'V19-DURABLE-IMAGE-CLOUD',uiLanguages:['bn','en'],features:{chat:true,stream:true,files:true,projects:true,webSearch:true,auth:true,cloudHistory:true,guestDailyLimit:null,guestChatUnlimited:true,vision:true,attachments:true,imageGeneration:true,imageEditing:true,multiImageEditing:true,imageStreaming:true}}));
app.get('/api/saba/attachment-capabilities',(_req,res)=>res.json({ok:true,version:'V19-DURABLE-IMAGE-CLOUD',enabled:Boolean(client),transport:'file_id',modes:['image','pdf','document','spreadsheet','text'],maxFileMb:20}));
...
app.get('/api/saba/image/file-download',async(req,res)=>{
  const id=rid();
  try{
    ...
    const ab=await r.arrayBuffer();
    res.set({'Content-Type':mime,'Content-Disposition':`attachment; filename="${filename.replace(/"/g,'')}"`,'Cache-Control':'no-store','Content-Length':String(ab.byteLength),'X-Content-Type-Options':'nosniff','X-SABA-Request-ID':id});
    res.send(Buffer.from(ab));
  }catch(e){
    console.error(`[${id}] /api/saba/image/file-download`,e?.message||e);
