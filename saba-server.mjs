...
const IMAGE_CLOUD_BUCKET=String(process.env.SABA_IMAGE_BUCKET||'saba-images').trim();
let imageCloudReadyPromise=null;
async function ensureImageCloud(){
  if(!admin)return false;
  if(imageCloudReadyPromise)return imageCloudReadyPromise;
  imageCloudReadyPromise=(async()=>{
    try{
      const got=await admin.storage.getBucket(IMAGE_CLOUD_BUCKET);
      if(got?.data)return true;
      const created=await admin.storage.createBucket(IMAGE_CLOUD_BUCKET,{public:false,fileSizeLimit:'60MB'});
      if(created?.error && !/already exists/i.test(String(created.error.message||'')))throw created.error;
      return true;
    }catch(e){
      console.warn('SABA image cloud storage unavailable:',e?.message||e);
      return false;
    }
  })();
  return imageCloudReadyPromise;
}
function safePathPart(v,fallback='x'){
  const x=String(v||'').replace(/[^A-Za-z0-9._-]/g,'_').slice(0,120);
  return x||fallback;
}
async function persistImageCloud(buffer,mime,meta={}){
  if(!admin)return null;
  if(!(await ensureImageCloud()))return null;
  const ext=mime==='image/jpeg'?'jpg':mime==='image/webp'?'webp':'png';
  const assetId=String(meta.asset_id||crypto.randomUUID());
  const clientPart=safePathPart(meta.client_id,'anonymous');
  const chatPart=safePathPart(meta.chat_id,'unassigned');
  const key=`generated/${clientPart}/${chatPart}/${assetId}.${ext}`;
  const up=await admin.storage.from(IMAGE_CLOUD_BUCKET).upload(key,buffer,{contentType:mime,upsert:false,cacheControl:'31536000'});
  if(up?.error)throw up.error;
  return {cloud_key:key,asset_id:assetId};
}
async function persistImageRecord(meta={}){
  if(!admin)return false;
  try{
    const row={asset_id:String(meta.asset_id||crypto.randomUUID()),client_id:String(meta.client_id||'anonymous'),chat_id:String(meta.chat_id||''),cloud_key:String(meta.cloud_key||''),openai_file_id:String(meta.openai_file_id||''),kind:String(meta.kind||'generated'),mime_type:String(meta.mime_type||'image/png'),output_format:String(meta.output_format||'png'),size:Number(meta.size||0),created_at:new Date(Number(meta.createdAt||Date.now())).toISOString()};
    const {error}=await admin.from('saba_image_assets').upsert(row,{onConflict:'asset_id'});
    if(error){console.warn('SABA image DB record unavailable:',error.message||error);return false;}
    return true;
  }catch(e){console.warn('SABA image DB record failed:',e?.message||e);return false}
}
async function fetchCloudImageBuffer(cloudKey){
  if(!admin)throw Object.assign(new Error('Cloud image storage is not configured.'),{status:503});
  const key=String(cloudKey||'').trim();
  if(!key||key.includes('..'))throw Object.assign(new Error('Invalid cloud image key.'),{status:400});
  const {data,error}=await admin.storage.from(IMAGE_CLOUD_BUCKET).download(key);
  if(error)throw Object.assign(new Error(error.message||'Cloud image could not be loaded.'),{status:404});
  const ab=await data.arrayBuffer();
  const ext=key.toLowerCase().endsWith('.jpg')||key.toLowerCase().endsWith('.jpeg')?'image/jpeg':key.toLowerCase().endsWith('.webp')?'image/webp':'image/png';
  return {buffer:Buffer.from(ab),mime:ext,name:key.split('/').pop()||'saba-image.png'};
}
...
const audioUpload=multer({storage:multer.memoryStorage(),limits:{fileSize:AUDIO_MAX_SIZE},fileFilter:(_req,file,cb)=>cb(null,String(file.mimetype||'').startsWith('audio/')||/webm|wav|mpeg|mp4|ogg|m4a/i.test(String(file.mimetype||'')))});
...
const MODEL_CATALOG=Object.freeze({
 'luis-1.2':()=>String(process.env.SABA_MODEL_LUIS_1_2||model).trim()
});
function resolveModel(key){const fn=MODEL_CATALOG[String(key||'luis-1.2')];return fn?fn():model}
function modeInstruction(mode){
 const m=String(mode||'chat');
 if(m==='coding')return '\nACTIVE MODE: CODING. Act as a senior software engineer. Prioritize correct, runnable code, debugging, architecture, concise explanations, and secure implementation. When the user asks for code, provide complete code when practical and do not replace implementation with vague advice.';
 if(m==='research')return '\nACTIVE MODE: RESEARCH. Research the user’s question using current web information when needed. Prefer primary/official sources, distinguish verified facts from inference, and cite or summarize sources when available.';
 return '\nACTIVE MODE: SABA CHAT. Handle general conversation, writing, study, planning, analysis, translation, and everyday tasks naturally.';
}
...
async function requestOf(body,req,stream=false){
 ...
 const p={model:resolveModel(body?.model_key),instructions:instructionsOf(body)+modeInstruction(body?.mode)+memoryNote,input:inputOf(body,visualMemories),max_output_tokens:body?.mode==='coding'?5000:2500};
...
async function resolveImageInput(input,index=0){
  ...
  if(raw.cloud_key){
    const f=await fetchCloudImageBuffer(raw.cloud_key);
...
function imagePayloadFromResponse(response,meta={}){
  ...
  return {
    ...
    image:{
      ...
      size:String(meta.size||item.size||'auto'),
      revised_prompt:item.revised_prompt||null,
      file_id:meta.file_id||null,
      cloud_key:meta.cloud_key||null,
...
async function persistGeneratedImage(response,meta={}){
  const item=response?.data?.[0]||{};
  const b64=String(item.b64_json||'').trim();
  if(!b64)return {file_id:null,cloud_key:null,asset_id:null};
  const format=String(meta.output_format||'png').toLowerCase();
  const mime=format==='jpeg'?'image/jpeg':format==='webp'?'image/webp':'image/png';
  const buffer=Buffer.from(b64,'base64');
  let fileId=null,cloud=null;
  try{
    const file=await toFile(buffer,`saba-generated-${Date.now()}.${format==='jpeg'?'jpg':format}`,{type:mime});
    const uploaded=await client.files.create({file,purpose:'assistants'});
    fileId=String(uploaded?.id||'')||null;
  }catch(e){console.warn('Generated OpenAI image persistence failed:',e?.message||e)}
  try{
    cloud=await persistImageCloud(buffer,mime,meta);
    if(cloud)await persistImageRecord({asset_id:cloud.asset_id,client_id:meta.client_id,chat_id:meta.chat_id,cloud_key:cloud.cloud_key,openai_file_id:fileId,kind:meta.kind||'generated',mime_type:mime,output_format:format,size:buffer.length,createdAt:Date.now()});
  }catch(e){console.warn('Generated cloud image persistence failed:',e?.message||e)}
  return {file_id:fileId,cloud_key:cloud?.cloud_key||null,asset_id:cloud?.asset_id||null};
}

async function persistReferenceImage(dataUrl,meta={}){
  const parts=dataUrlParts(dataUrl);
  if(!parts)throw Object.assign(new Error('Invalid reference image data.'),{status:400});
  const buffer=Buffer.from(parts.base64,'base64');
  if(buffer.length>25*1024*1024)throw Object.assign(new Error('Reference image is too large.'),{status:413});
  let fileId=null,cloud=null;
  const ext=parts.mime==='image/jpeg'?'jpg':parts.mime==='image/webp'?'webp':parts.mime==='image/gif'?'gif':'png';
  try{
    const file=await toFile(buffer,String(meta.name||`saba-reference-${Date.now()}.${ext}`),{type:parts.mime});
    const uploaded=await client.files.create({file,purpose:'assistants'});
    fileId=String(uploaded?.id||'')||null;
  }catch(e){console.warn('Reference OpenAI persistence failed:',e?.message||e)}
  try{
    cloud=await persistImageCloud(buffer,parts.mime,{...meta,kind:'reference'});
    if(cloud)await persistImageRecord({asset_id:cloud.asset_id,client_id:meta.client_id,chat_id:meta.chat_id,cloud_key:cloud.cloud_key,openai_file_id:fileId,kind:'reference',mime_type:parts.mime,output_format:ext,size:buffer.length,createdAt:Date.now()});
  }catch(e){console.warn('Reference cloud persistence failed:',e?.message||e)}
  if(!fileId&&!cloud?.cloud_key)throw Object.assign(new Error('Reference image could not be saved to persistent storage.'),{status:503});
  return {file_id:fileId,cloud_key:cloud?.cloud_key||null,asset_id:cloud?.asset_id||null};
}
...
async function editImage(body){
  ...
  const options=normalizeImageOptions(body);
  const referenceInstruction='Use the supplied image(s) as the primary visual reference. Preserve the identity and recognizable facial features of people, including face shape, eyes, nose, mouth, skin tone, hairstyle, age and overall proportions, unless the user explicitly asks to change them. Preserve all other important visual details unless the user asks to change them. Make only the requested changes and keep everything else as close to the source as possible. Do not invent a different person or unnecessarily regenerate the subject.';
  const editPrompt=`${referenceInstruction}\n\nUser instruction: ${prompt}`;
  const request={
    model:IMAGE_MODEL,
...
app.get('/',(_req,res)=>res.json({ok:true,service:'SABA Universal AI',version:'V23-COMPOSER-MODES',model,keyConfigured:Boolean(client),authConfigured:Boolean(supabaseUrl&&supabaseAnonKey),guestLimitConfigured:true,visionEnabled:Boolean(client),attachmentEnabled:true,imageGenerationEnabled:Boolean(client),imageModel:IMAGE_MODEL,imageCloudPersistence:Boolean(admin)}));
app.get('/health',(_req,res)=>res.json({ok:true,service:'SABA Universal AI',version:'V23-COMPOSER-MODES',model,keyConfigured:Boolean(client),authConfigured:Boolean(supabaseUrl&&supabaseAnonKey),guestLimitConfigured:true,visionEnabled:Boolean(client),attachmentEnabled:true,imageGenerationEnabled:Boolean(client),imageModel:IMAGE_MODEL,timestamp:new Date().toISOString()}));
app.get('/api/saba/config',(_req,res)=>res.json({ok:true,version:'V23-COMPOSER-MODES',uiLanguages:['bn','en'],features:{chat:true,stream:true,files:true,projects:true,webSearch:true,auth:true,cloudHistory:true,guestDailyLimit:null,guestChatUnlimited:true,vision:true,attachments:true,imageGeneration:true,imageEditing:true,multiImageEditing:true,imageStreaming:true,tools:true,codingMode:true,researchMode:true,voiceInput:true,modelSelector:true}}));
app.get('/api/saba/attachment-capabilities',(_req,res)=>res.json({ok:true,version:'V23-COMPOSER-MODES',enabled:Boolean(client),transport:'file_id',modes:['image','pdf','document','spreadsheet','text'],maxFileMb:20}));
...
app.post('/api/saba/transcribe',audioUpload.single('audio'),async(req,res)=>{
 ...
 try{
  ...
  if(!req.file)return res.status(400).json({ok:false,error:'Audio was not provided.',requestId:id});
  const file=await toFile(req.file.buffer,req.file.originalname||'saba-voice.webm',{type:req.file.mimetype||'audio/webm'});
  const out=await client.audio.transcriptions.create({model:String(process.env.SABA_TRANSCRIBE_MODEL||'gpt-transcribe'),file});
  const text=String(out?.text||'').trim();
  if(!text)return res.status(502).json({ok:false,error:'No speech was detected.',requestId:id});
  res.json({ok:true,text,language:out?.language||null,requestId:id,model:String(process.env.SABA_TRANSCRIBE_MODEL||'gpt-transcribe')});
 }catch(e){
  console.error(`[${id}] /api/saba/transcribe`,e?.message||e);
  const status=Number(e?.status)>=400&&Number(e?.status)<600?Number(e.status):502;
  res.status(status).json({ok:false,error:e?.message||'Voice transcription failed.',requestId:id});
 }
});

app.get('/api/saba/models',(_req,res)=>res.json({ok:true,models:[{id:'luis-1.2',label:'Luis 1.2',default:true,configured_model:resolveModel('luis-1.2')}]}));
...
app.post('/api/saba/image/generate',async(req,res)=>{
  ...
  try{
    ...
    const persisted=await persistGeneratedImage(response,{...options,client_id:clientId(req),chat_id:String(req.body?.chat_id||''),kind:'generated'});
    const out=imagePayloadFromResponse(response,{...options,...persisted});
...
// Persist a reference image once so later edits in the same conversation can reuse it.
// The source is stored as an OpenAI file with a downloadable purpose, never `user_data`.
app.post('/api/saba/image/reference',async(req,res)=>{
  const id=rid();res.set('X-SABA-Request-ID',id);
  try{
    if(!requireKey(res,id))return;
    const dataUrl=String(req.body?.data_url||'').trim();
    if(!dataUrl)return res.status(400).json({ok:false,error:'Reference image data is required.',requestId:id});
    const fileId=await persistReferenceImage(dataUrl,{name:req.body?.name||'reference-image'});
    if(!fileId)throw new Error('Reference image could not be saved.');
    res.json({ok:true,file_id:fileId,requestId:id});
  }catch(e){
    console.error(`[${id}] /api/saba/image/reference`,e?.message||e);
    res.status(imageErrorStatus(e)).json({ok:false,error:e?.message||'Reference image could not be saved.',requestId:id});
  }
});
...
// Direct download endpoint for generated images. It keeps the OpenAI API key
// server-side and returns the bytes as an attachment so browser download clicks
// work even when the frontend and Render API are on different origins.
// Unified attachment endpoint. The frontend uses asset_id first so downloads
// remain independent of browser image-cache URLs and OpenAI file retention.
app.get('/api/saba/image/download',async(req,res)=>{
  const id=rid();
  try{
    const assetId=String(req.query?.asset_id||'').trim();
    const cloudKey=String(req.query?.cloud_key||'').trim();
    const fileId=String(req.query?.file_id||'').trim();
    let f=null;
    if(assetId) f=await fetchCloudImageByAssetId(assetId);
    else if(cloudKey) f=await fetchCloudImageBuffer(cloudKey);
    else if(fileId){
      if(!client)throw Object.assign(new Error('OPENAI_API_KEY is not configured.'),{status:503});
      if(!/^file-[A-Za-z0-9_-]+$/.test(fileId))throw Object.assign(new Error('Invalid image file ID.'),{status:400});
      ...
      if(!r.ok){const detail=await r.text().catch(()=> '');throw Object.assign(new Error(detail||'Image file could not be downloaded.'),{status:Number(r.status)||502});}
      const ab=await r.arrayBuffer();
      const mime=String(r.headers.get('content-type')||'image/png').split(';')[0].trim();
      const ext=mime==='image/jpeg'?'jpg':mime==='image/webp'?'webp':'png';
      f={buffer:Buffer.from(ab),mime,name:`saba-image.${ext}`};
    } else throw Object.assign(new Error('No image asset was supplied.'),{status:400});
    const raw=String(req.query?.name||f.name||`saba-image-${Date.now()}.png`).trim();
    const name=(raw.replace(/[\\/\r\n"]/g,'_').slice(0,180)||f.name||'saba-image.png');
    res.status(200).set({
    ...
    console.error(`[${id}] /api/saba/image/download`,e?.message||e);
    res.status(Number(e?.status)||404).json({ok:false,error:e?.message||'Image download failed.',requestId:id});
  }
});

// Download compatibility: keep attachment responses usable from browser and Android WebView.
app.options('/api/saba/image/file/:id/download',(req,res)=>res.sendStatus(204));
app.get('/api/saba/image/file/:id/download',async(req,res)=>{
  const id=rid();
  try{
    ...
    const r=await fetch(`https://api.openai.com/v1/files/${encodeURIComponent(fileId)}/content`,{headers:{Authorization:`Bearer ${openaiKey}`}});
    if(!r.ok){const detail=await r.text().catch(()=> '');return res.status(Number(r.status)||502).json({ok:false,error:detail||'Image file could not be downloaded.',requestId:id})}
    const mime=String(r.headers.get('content-type')||'image/png').split(';')[0].trim();
    const ext=mime==='image/jpeg'?'jpg':mime==='image/webp'?'webp':mime==='image/gif'?'gif':'png';
    const requested=String(req.query?.name||'').trim().replace(/[^A-Za-z0-9._-]+/g,'_').slice(0,120);
    const filename=requested||`saba-image-${Date.now()}.${ext}`;
    const ab=await r.arrayBuffer();
    res.set({'Content-Type':mime,'Content-Disposition':`attachment; filename=\"${filename.replace(/\"/g,'')}\"`,'Cache-Control':'no-store','X-SABA-Request-ID':id});
    res.send(Buffer.from(ab));
  }catch(e){
    console.error(`[${id}] /api/saba/image/file/:id/download`,e?.message||e);
    res.status(imageErrorStatus(e)).json({ok:false,error:e?.message||'Image file could not be downloaded.',requestId:id});
  }
});
  ...
  try{
    ...
    for await(const event of stream){
      ...
      if(b64){
        ...
        const payload={data_url:`data:${mime};base64,${b64}`,mime_type:mime,output_format:options.output_format,size:options.size};
        if(event?.type==='image_generation.partial_image')send({type:'image_partial',...payload,index:event?.partial_image_index??null});
        else {
          const generatedFile=await persistGeneratedImage({data:[{b64_json:b64}]},{...options,client_id:clientId(req),chat_id:String(req.body?.chat_id||''),kind:'generated'});
          finalPayload={...payload,...generatedFile};
      ...
      if(event?.type==='image_generation.completed'&&finalPayload===null){
        ...
        if(maybe){
          const mime=options.output_format==='jpeg'?'image/jpeg':options.output_format==='webp'?'image/webp':'image/png';
          finalPayload={data_url:`data:${mime};base64,${maybe}`,mime_type:mime,output_format:options.output_format,size:options.size};
          send({type:'image',...finalPayload});
        }
      }
    }
...
// Robust query-string cloud routes. These avoid path/wildcard encoding problems
// in browsers, Android WebView, and reverse proxies when cloud keys contain slashes.
app.get('/api/saba/image/cloud',async(req,res)=>{
  const id=rid();
  try{
    const key=String(req.query?.key||'').trim();
    const f=await fetchCloudImageBuffer(key);
    res.set({'Content-Type':f.mime,'Cache-Control':'private, max-age=31536000','Content-Length':String(f.buffer.length),'X-SABA-Request-ID':id});
    res.send(f.buffer);
  }catch(e){console.error(`[${id}] /api/saba/image/cloud`,e?.message||e);res.status(Number(e?.status)||404).json({ok:false,error:e?.message||'Image could not be loaded.',requestId:id})}
});
app.get('/api/saba/image/cloud/download',async(req,res)=>{
  const id=rid();
  try{
    const key=String(req.query?.key||'').trim();
    const f=await fetchCloudImageBuffer(key);
    const name=String(req.query?.name||f.name).replace(/[\\/\\r\\n\"]/g,'_').slice(0,180)||f.name;
    res.set({'Content-Type':f.mime,'Content-Disposition':`attachment; filename="${name}"`,'Cache-Control':'private, no-store','Content-Length':String(f.buffer.length),'X-SABA-Request-ID':id});
    res.send(f.buffer);
  }catch(e){console.error(`[${id}] /api/saba/image/cloud/download`,e?.message||e);res.status(Number(e?.status)||404).json({ok:false,error:e?.message||'Image download failed.',requestId:id})}
});
async function fetchCloudImageByAssetId(assetId){
  if(!admin)throw Object.assign(new Error('Cloud image storage is not configured.'),{status:503});
  const aid=String(assetId||'').trim();
  if(!aid)throw Object.assign(new Error('Invalid image asset ID.'),{status:400});
  const {data,error}=await admin.from('saba_image_assets').select('cloud_key,mime_type,output_format').eq('asset_id',aid).maybeSingle();
  if(error)throw Object.assign(new Error(error.message||'Image record could not be loaded.'),{status:404});
  if(!data?.cloud_key)throw Object.assign(new Error('Image asset has no cloud storage key.'),{status:404});
  return await fetchCloudImageBuffer(data.cloud_key);
}
app.get('/api/saba/image/asset/:assetId',async(req,res)=>{
  const id=rid();
  try{
    const f=await fetchCloudImageByAssetId(req.params.assetId);
    res.set({'Content-Type':f.mime,'Cache-Control':'private, max-age=31536000','Content-Length':String(f.buffer.length),'X-SABA-Request-ID':id});
    res.send(f.buffer);
  }catch(e){console.error(`[${id}] /api/saba/image/asset`,e?.message||e);res.status(Number(e?.status)||404).json({ok:false,error:e?.message||'Image could not be loaded.',requestId:id})}
});
app.get('/api/saba/image/asset/:assetId/download',async(req,res)=>{
  const id=rid();
  try{
    const f=await fetchCloudImageByAssetId(req.params.assetId);
    const name=String(req.query?.name||f.name).replace(/[\\/\\r\\n\"]/g,'_').slice(0,180)||f.name;
    res.set({'Content-Type':f.mime,'Content-Disposition':`attachment; filename="${name}"`,'Cache-Control':'private, no-store','Content-Length':String(f.buffer.length),'X-SABA-Request-ID':id});
    res.send(f.buffer);
  }catch(e){console.error(`[${id}] /api/saba/image/asset/download`,e?.message||e);res.status(Number(e?.status)||404).json({ok:false,error:e?.message||'Image download failed.',requestId:id})}
});

app.get('/api/saba/image/cloud/:key(*)/download',async(req,res)=>{
  const id=rid();
  try{
    ...
    res.set({'Content-Type':f.mime,'Content-Disposition':`attachment; filename="${name}"`,'Cache-Control':'private, no-store','Content-Length':String(f.buffer.length),'X-SABA-Request-ID':id});
    res.send(f.buffer);
  }catch(e){console.error(`[${id}] cloud image download`,e?.message||e);res.status(Number(e?.status)||404).json({ok:false,error:e?.message||'Image download failed.',requestId:id})}
});
app.get('/api/saba/image/cloud/:key(*)',async(req,res)=>{
  const id=rid();
  try{
    const key=String(req.params.key||'');
    const f=await fetchCloudImageBuffer(key);
    res.set({'Content-Type':f.mime,'Cache-Control':'public, max-age=31536000, immutable','Content-Length':String(f.buffer.length),'X-SABA-Request-ID':id});
    res.send(f.buffer);
  }catch(e){console.error(`[${id}] cloud image`,e?.message||e);res.status(Number(e?.status)||404).json({ok:false,error:e?.message||'Image could not be loaded.',requestId:id})}
});
