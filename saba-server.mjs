...
function clientId(req){const raw=String(req.get('X-SABA-Client-ID')||'');return /^[A-Za-z0-9_-]{8,120}$/.test(raw)?raw:'anonymous'}
...
app.post('/api/saba/file',upload.single('file'),async(req,res)=>{
 ...
 try{
   ...
   res.json({ok:true,name:req.file.originalname,mime_type:mime,size:req.file.size,file_id:uploaded.id,file_status:uploaded.status||'uploaded',client_id:clientId(req),temporary:true,requestId:id});
 }catch(e){
...
app.delete('/api/saba/files/:id',async(req,res)=>{
 ...
 try{
   ...
   await client.files.delete(fileId);res.json({ok:true,requestId:id});
